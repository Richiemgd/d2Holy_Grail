...
def item_ui(slot, type):
    if slot != 'Sets':
        ...
        set_items = hg.get_items_by_value('Item Group 1', type, hg.get_all_sets)
...
def update_stats():
    ...
    window['-ITEMS-'].Update(
    f'Normal Grail {hg.get_normal_grail_completed_perc}% completed: {len(hg.get_found_item_names)} of {len(hg.get_normal_grail_names)} items found')
    window['-RUNES-'].Update(
        f'Runes Grail {hg.get_runes_grail_completed_perc}% completed: {len(hg.get_found_rune_names)} of {len(hg.get_runes_grail_names)} runes found')
    window['-NOT_FOUND_ITEMS-'].Update(hg.get_not_found_item_names)
...
sets1 = [[sg.Text('Sets', justification='c')],
         [sg.Column(item_ui('Sets', f'{name}')) for name in hg.get_all_sets_names[0:6]],
         [sg.Column(item_ui('Sets', f'{name}')) for name in hg.get_all_sets_names[6:12]],
         [sg.Column(item_ui('Sets', f'{name}')) for name in hg.get_all_sets_names[12:18]]]
 
sets2 = [[sg.Text('Sets', justification='c')],
         [sg.Column(item_ui('Sets', f'{name}')) for name in hg.get_all_sets_names[18:24]],
         [sg.Column(item_ui('Sets', f'{name}')) for name in hg.get_all_sets_names[24:30]],
...
runes += [[sg.Checkbox(name, enable_events=True,
                       key=f'{name} checkBox')] for name in hg.get_runes_grail_names]
...
stats = [[sg.Text('Holy Grail', justification='c')],
         ...
         [sg.Input(enable_events=True, key='-SEARCHBOX-'), sg.Button('Add Item',
                                                                     size=(10, 1))],
         [sg.Listbox(hg.get_not_found_item_names, enable_events=True,
                     key='-NOT_FOUND_ITEMS-')]
         ]
...
[window[f'{name} checkBox'].Update(True) for name in hg.get_all_found_names]
...
while True:             # Event Loop
    ...
    if event == '-SEARCHBOX-':
        ...
        new_values = [x for x in hg.get_not_found_item_names if search in x.lower()]
